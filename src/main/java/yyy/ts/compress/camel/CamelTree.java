...
public class CamelTree {
    ...
    private int writeFirst(long value) {
        ...
        size += 64;
        compressVal.put("compressInt", convertToBinary(0, 2));
        return size;
    }
...
//         ***** 对于值查询就是统一成一颗树
    ...
    private int compressDecimalValue(BigDecimal decimal_value) {
        ...
        compressVal.put("decimalCount", decimalToBinary(decimal_count, 2));
        size += 2;
...
//            int trailingZeros = Long.numberOfTrailingZeros(xor);
            ...
            compressVal.put("xorFlag", decimalToBinary(1, 1));
            compressVal.put("xorVal", binaryLongToBinary(xor, Long.numberOfTrailingZeros(xor)));
        } else {  // m就为原来的值
            out.writeBit(false);
            size += 1;
            compressVal.put("xorFlag", decimalToBinary(0, 1));
        }
        ...
        if (decimal_count <= 1) { // 如果是1 直接往后读decimal_count+1位
            ...
            compressVal.put("compressDecimal", decimalToBinary(m_int, decimal_count+1));
        } else if (decimal_count ==2) {
            if (m_int < 8) {
                out.writeInt(0, 1);
                out.writeInt(m_int, 3);
                size += 4;
                compressVal.put("compressDecimal", decimalToBinary(m_int, 3));
            }  else {
                out.writeInt(1, 1);
                out.writeInt(m_int, 5);
                size += 6;
                compressVal.put("compressDecimal", decimalToBinary(m_int, 5));
            }
            ...
            if (m_int < 2) {
                ...
                size += 3;
                compressVal.put("compressDecimal", decimalToBinary(m_int, 2));
            }else if (m_int < 8){
                out.writeInt(1, 2);
                out.writeInt(m_int, 3);
                size += 5;
                ...
                out.writeInt(m_int, 5);
                size += 7;
                compressVal.put("compressDecimal", decimalToBinary(m_int, 4));
            }else {
                out.writeInt(3, 2);
                out.writeInt(m_int, mValueBits[decimal_count-1]);
                size += 2;
                size += mValueBits[decimal_count-1];
                compressVal.put("compressDecimal", decimalToBinary(m_int, mValueBits[decimal_count-1]));
            }
            ...
            if (m_int < 16) {
                ...
                size += 8;
                compressVal.put("compressDecimal", decimalToBinary(m_int, 6));
            }else if (m_int < 256) {
                out.writeInt(2, 2);
                out.writeInt(m_int, 8);
                size += 10;
                compressVal.put("compressDecimal", decimalToBinary(m_int, 8));
            }else {
                out.writeInt(3, 2);
                out.writeInt(m_int, mValueBits[decimal_count-1]);
                size += 2;
